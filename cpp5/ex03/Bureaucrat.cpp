...
/*   Updated: 2025/06/04 21:04:03 by jeremie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Bureaucrat::signForm(AForm &form) const
{
	try
	{
		if (form.beSigned(*this))
			std::cout << _name << " couldn't sign " << form.getName() << " because it was already signed.\n";
		else
	...
	catch (std::exception &e)
	{
		...
		if (error == "Grade too low to sign.\n")
...
{
{
	...
	catch (std::exception &e)
	{
		std::string error(e.what());
		if (error == "Grade too low to sign.\n")
			std::cout << _name << " couldn't execute " << form.getName() << " because his grade is too low.\n";
		else if (error == "Form not signed\n")
			std::cout << _name << " couldn't execute " << form.getName() << " because it hasn't been signed yet.\n";
		else
			std::cout << "error: " << error;				
	}
}
